...
export function Dashboard() {
  const { t } = useLanguage();
  const { vehicles, expenses } = useData();
  ...
  const expenseData = useMemo(() => {
    ...
    for (const e of expenses) {
      ...
      if (key) byMonth[key] = (byMonth[key] || 0) + e.amount;
    }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="col-span-2 p-5 bg-zinc-900 border border-zinc-800 rounded-2xl flex flex-col">
          <span className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-6">{t.dashboard.expenseOverview}</span>
        ...
        <div className="col-span-1 p-5 bg-zinc-900 border border-zinc-800 rounded-2xl flex flex-col">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold uppercase tracking-widest text-zinc-500">{t.dashboard.recentExpenses}</span>
          </div>
          <div className="space-y-3 flex-1 overflow-y-auto pr-1">
            {[...expenses].sort((a,b) => b.date.localeCompare(a.date)).slice(0, 5).map((expense) => {
               // Assign color based on type
               let borderColor = 'border-blue-500';
               if (expense.type === 'fuel') borderColor = 'border-emerald-500';
               else if (expense.type === 'maintenance') borderColor = 'border-amber-500';
               else if (expense.type === 'toll') borderColor = 'border-purple-500';

               return (
                 <div key={expense.id} className={`p-3 bg-zinc-950 border-l-2 ${borderColor} rounded flex justify-between`}>
                   <div>
                     <p className="text-xs font-bold text-zinc-200">{expense.vehicleId}: {t.common[expense.type.toLowerCase() as keyof typeof t.common] || expense.type}</p>
                     <p className="text-[10px] text-zinc-500 mt-1">{expense.description} • {expense.date}</p>
                   </div>
